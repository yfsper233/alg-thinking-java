 ...
 */
public class DSU_1_寻找图中是否存在路径 {
    public static void main(String[] args) {
        int[][] edges = {{0, 1}, {1, 2}, {2, 0}};
        int n = 3;
        int source = 0;
        int destination = 2;
        boolean res = findPathExits(edges, n, source, destination);
    }

    private static boolean findPathExits(int[][] edges, int n, int source, int destination) {
        DisjointSetUnion union = new DisjointSetUnion(n);
        for (int i = 0; i < edges.length; i++) {
            union.union(edges[i][0], edges[i][1]);
        }
        return union.isConnected(source, destination);
    }
}
